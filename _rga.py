...
from evolutionary.gas._utils import rands, sbc, polynomial_mutation, elitism, sus, rws, rbs
...
class RGA:
    ...
    def __init__(self,
                 ...
                 mutation='', UR=0.3,  # mutation strategy and setup params
    ...
    def solve(self):
        ...
        for nit in range(1, self.iterations + 1):
            ...
            for p in range(0, self.popsize, 2):
                if self.rnd.rand(1,1) < self.CR:
                    Child1, Child2 = sbc(self.POP[mates[p]], self.POP[mates[p+1]], self.mu, self.rnd)
                    if self.rnd.rand(1, 1) < self.UR:
                        Child1 = polynomial_mutation(Child1, np.array([0]*self.dims), np.array([1]*self.dims), self.mum, self.rnd)
                        Child2 = polynomial_mutation(Child2, np.array([0]*self.dims), np.array([1]*self.dims), self.mum, self.rnd)
                else:
                    Child1 = polynomial_mutation(self.POP[mates[p]], np.array([0]*self.dims), np.array([1]*self.dims), self.mum, self.rnd)
                    Child2 = polynomial_mutation(self.POP[mates[p+1]], np.array([0]*self.dims), np.array([1]*self.dims), self.mum, self.rnd)

                offspring[p] = Child1
                offspring[p+1] = Child2

            # bound constrain
            mask = np.where((offspring > 1) | (offspring < 0))
            offspring[mask] = self.rnd.uniform(size=mask[0].shape)
...
if __name__ == '__main__':
    ...
    symbol = "ackley"
    settings = f_settings(dims=1000)
    bounds = settings[symbol]["bounds"]
    from utils import _FunctionWrapper

    alg = RGA(func=_FunctionWrapper(eval(symbol), bounds), popsize=50, iterations=2000)
    alg.solve()
    print(alg.x)
